...
`ee.Date`を使って日付オブジェクトを作成する．
`start`は開始日，`end`は終了日とする．
また，`lon`と`lat`に経度と緯度を格納し，`ee.Geometry.Point`で点オブジェクトを作成する．
...
`ee.ImageCollection`を用いて画像検索を行い，結果を`ImageCollection`に格納する．
